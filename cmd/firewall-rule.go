...
var firewallRuleCmd = &cobra.Command{
	...
	PreRun: func(cmd *cobra.Command, args []string) {
		...
		if !install.InArray(install.FirewallRuleConfig.Mode, []string{"add", "del", "install", "uninstall"}) {
			err := cmd.Help()
			if err != nil {
				return
			}
...
func init() {
	...
	firewallRuleCmd.Flags().StringVar(&install.FirewallRuleConfig.Mode, "mode", "", "add|del|install|uninstall")
